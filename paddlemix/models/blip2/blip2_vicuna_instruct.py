...
class Blip2VicunaInstruct(Blip2Base):
    def __init__(
        ...
        self.load_pretrained(vision_name_or_path="paddlemix/blip2-stage2/eva_vit_g")
    ...
    def concat_text_input_output(self, input_ids, input_atts, output_ids, output_atts):
        ...
        for i in range(input_ids.shape[0]):
            ...
            llm_tokens["input_ids"].append(
                paddle.concat([input_ids[i][:this_input_ones], output_ids[i][1:], input_ids[i][this_input_ones:]])
            )
            llm_tokens["attention_mask"].append(
                paddle.concat([input_atts[i][:this_input_ones], output_atts[i][1:], input_atts[i][this_input_ones:]])
            )
