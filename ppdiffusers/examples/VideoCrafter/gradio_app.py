...
def videocrafter_demo(result_dir="./tmp/"):
    ...
    with gr.Blocks(analytics_enabled=False) as videocrafter_iface:
        gr.Markdown(
            "<div align='center'> <h2> VideoCrafter2: Overcoming Data Limitations for High-Quality Video Diffusion Models </span> </h2>                      <a style='font-size:18px;color: #000000' href='https://github.com/AILab-CVC/VideoCrafter'> Github </div>"
        )
        with gr.Tab(label="Text2Video"):
            with gr.Column():
                with gr.Row():  # .style(equal_height=False):
                    with gr.Column():
                        ...
                        with gr.Row():
                            steps = gr.Slider(
                                minimum=1, maximum=60, step=1, elem_id="steps", label="Sampling steps", value=50
                            )
                            eta = gr.Slider(minimum=0.0, maximum=1.0, step=0.1, label="ETA", value=1.0, elem_id="eta")
                        with gr.Row():
                            cfg_scale = gr.Slider(
                                minimum=1.0, maximum=30.0, step=0.5, label="CFG Scale", value=12.0, elem_id="cfg_scale"
                            )
            ...
            send_btn.click(
                fn=text2video.get_prompt, inputs=[input_text, steps, cfg_scale, eta, fps], outputs=[output_video_1]
            )
...
if __name__ == "__main__":
    ...
    # videocrafter_iface.queue(default_concurrency_limit=1, max_size=10)
