...
def get_tokenizer(model_name: str = "", context_length: Optional[int] = None, **kwargs):
    if model_name.startswith(HF_HUB_PREFIX):
        ...
        try:
            ...
            tokenizer = HFTokenizer(model_name, context_length=context_length or DEFAULT_CONTEXT_LENGTH, **kwargs)
            return tokenizer
    else:
    ...
    if "hf_tokenizer_name" in text_config:
        tokenizer = HFTokenizer(text_config["hf_tokenizer_name"], context_length=context_length, **tokenizer_kwargs)
    else:
...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    if "logit_bias" not in state_dict and model.logit_bias is not None:
        state_dict["logit_bias"] = paddle.zeros_like(x=state_dict["logit_scale"])
    position_id_key = "text.transformer.embeddings.position_ids"
    if position_id_key in state_dict and not hasattr(model, position_id_key):
        del state_dict[position_id_key]
    resize_pos_embed(state_dict, model)
    resize_text_pos_embed(state_dict, model)
    incompatible_keys = model.set_state_dict(state_dict=state_dict, use_structured_name=strict)
...
def create_model(
    ...
    **model_kwargs
    ...
    if pretrained and pretrained.lower() == "openai":
        ...
        # model = load_openai_model(model_name, precision=precision, device=
        ...
        if precision in ("fp16", "bf16"):
            dtype = "float16" if "fp16" in precision else "bfloat16"
            if is_timm_model:
                ...
                def _convert_ln(m):
                    if isinstance(m, LayerNormFp32):
                        m.weight.data = m.weight.data.to("float32")
                        m.bias.data = m.bias.data.to("float32")
