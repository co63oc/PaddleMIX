...
class SimpleTokenizer(object):
    def __init__(
        ...
        self.pat = re.compile(
            special + "|'s|'t|'re|'ve|'m|'ll|'d|[\\p{L}]+|[\\p{N}]|[^\\s\\p{L}\\p{N}]+", re.IGNORECASE
        )
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> paddle.int64:
...
def decode(output_ids: paddle.Tensor):
    output_ids = output_ids.cpu().numpy()
    return _tokenizer.decode(output_ids)


def tokenize(texts: Union[str, List[str]], context_length: int = DEFAULT_CONTEXT_LENGTH) -> paddle.int64:
...
def random_mask_tokenize(
    ...
    result = paddle.zeros(shape=[len(all_tokens), context_length], dtype="int64")
    for i, tokens in enumerate(all_tokens):
        tokens = paddle.to_tensor(data=tokens)
        num_tokens = len(tokens)
        if num_tokens > context_length - 2:
            num_keep = context_length - 2
            indices = paddle.randperm(n=len(tokens))
            indices = indices[:num_keep]
            if not shuffle:
                indices = paddle.sort(indices, axis=0)
...
def simple_mask_tokenize(
    texts: Union[str, List[str]], context_length: int, sot_token_id: int, eot_token_id: int, encode_fn: Callable
):
    all_tokens = [encode_fn(text) for text in texts]
    result = paddle.zeros(shape=[len(all_tokens), context_length], dtype="int64")
    for i, tokens in enumerate(all_tokens):
...
class HFTokenizer:
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> paddle.Tensor:
        if isinstance(texts, str):
            texts = [texts]
        context_length = context_length or self.context_length
        ...
        if self.strip_sep_token:
            input_ids = paddle.where(
                condition=input_ids == self.tokenizer.sep_token_id, x=paddle.zeros_like(x=input_ids), y=input_ids
            )
        return input_ids
...
class SigLipTokenizer:
    ...
    def __init__(self, tokenizer_name: str, context_length: Optional[int] = 64):
        if tokenizer_name in self.VOCAB_FILES:
            ...
            with tempfile.NamedTemporaryFile("wb") as dst:
                with fsspec.open(vocab_file, "rb") as src:
                    dst.write(src.read())
                self.tokenizer = transformers.T5TokenizerFast(dst.name, legacy=False)
        else:
            self.tokenizer = transformers.T5TokenizerFast(tokenizer_name, legacy=False)
