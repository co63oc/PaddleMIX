...
def merge_preprocess_dict(base: Union[PreprocessCfg, Dict], overlay: Dict):
...
class ResizeKeepRatio:
    ...
    def __call__(self, img):
        ...
        img = torchvision.transforms.functional.resize(img, size, self.interpolation)
        return img
...
def center_crop_or_pad(img: paddle.Tensor, output_size: List[int], fill=0) -> paddle.Tensor:
    ...
    if crop_width > image_width or crop_height > image_height:
        ...
        img = torchvision.transforms.functional.pad(img, padding_ltrb, fill=fill)
        _, image_height, image_width = torchvision.transforms.functional.get_dimensions(img)
        if crop_width == image_width and crop_height == image_height:
            return img
    crop_top = int(round((image_height - crop_height) / 2.0))
    crop_left = int(round((image_width - crop_width) / 2.0))
    return torchvision.transforms.functional.crop(img, crop_top, crop_left, crop_height, crop_width)
...
class color_jitter(object):
    ...
    def __init__(self, brightness=0.0, contrast=0.0, saturation=0.0, hue=0.0, p=0.8):
        assert 0.0 <= p <= 1.0
        self.p = p
        self.transf = torchvision.transforms.ColorJitter(
...
class gray_scale(object):
    ...
    def __init__(self, p=0.2):
        assert 0.0 <= p <= 1.0
        self.p = p
        self.transf = torchvision.transforms.Grayscale(num_output_channels=3)
...
def image_transform(
    ...
    interpolation_mode = (
        ...
        else torchvision.transforms.InterpolationMode.BICUBIC
    )
    ...
    if is_train:
        ...
        if use_timm:
            ...
            train_transform.extend([torchvision.transforms.ToTensor(), normalize])
        ...
        if resize_mode == "longest":
            ...
            transforms = [torchvision.transforms.Resize(image_size, interpolation=interpolation_mode)]
        else:
            assert resize_mode == "shortest"
            if not isinstance(image_size, (tuple, list)):
                image_size = image_size, image_size
            if image_size[0] == image_size[1]:
                transforms = [torchvision.transforms.Resize(image_size[0], interpolation=interpolation_mode)]
            else:
                transforms = [ResizeKeepRatio(image_size)]
            transforms += [torchvision.transforms.CenterCrop(image_size)]
        transforms.extend([_convert_to_rgb, torchvision.transforms.ToTensor(), normalize])
        return torchvision.transforms.Compose(transforms)


def image_transform_v2(
    cfg: PreprocessCfg, is_train: bool, aug_cfg: Optional[Union[Dict[str, Any], AugmentationCfg]] = None
):
