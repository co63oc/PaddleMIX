...
class AutoencoderKL(paddle.nn.Layer):
    def __init__(
        ...
        self.quant_conv = paddle.nn.Conv2D(
            in_channels=2 * ddconfig["z_channels"], out_channels=2 * embed_dim, kernel_size=1
        )
        self.post_quant_conv = paddle.nn.Conv2D(
            in_channels=embed_dim, out_channels=ddconfig["z_channels"], kernel_size=1
        )
        ...
        if colorize_nlabels is not None:
            ...
            self.register_buffer(name="colorize", tensor=paddle.randn(shape=[3, colorize_nlabels, 1, 1]))
    ...
    def configure_optimizers(self):
        ...
        opt_disc = paddle.optimizer.Adam(
            parameters=self.loss.discriminator.parameters(),
            learning_rate=lr,
            beta1=(0.5, 0.9)[0],
            beta2=(0.5, 0.9)[1],
            weight_decay=0.0,
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        ...
        if not only_inputs:
            ...
            log["samples"] = self.decode(
                randn_tensor_mod(shape=posterior.sample().shape, dtype=posterior.sample().dtype)
            )
    ...
    def to_rgb(self, x):
        ...
        if not hasattr(self, "colorize"):
            self.register_buffer(name="colorize", tensor=paddle.randn(shape=[3, tuple(x.shape)[1], 1, 1]).to(x))
        x = paddle.nn.functional.conv2d(x=x, weight=self.colorize)
        x = 2.0 * (x - x.min()) / (x.max() - x.min()) - 1.0
        return x


class IdentityFirstStage(paddle.nn.Layer):
