    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            x = paddle.nn.functional.interpolate(
                x=x, size=(tuple(x.shape)[2], tuple(x.shape)[3] * 2, tuple(x.shape)[4] * 2), mode="nearest"
            )
        else:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = paddle.nn.Sequential(
            normalization(channels), paddle.nn.Silu(), conv_nd(dims, channels, self.out_channels, 3, padding=1)
        )
        ...
        self.out_layers = paddle.nn.Sequential(
            ...
            zero_module(
                paddle.nn.Conv2D(
                    in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=3, padding=1
                )
            ),
        )
...
class TemporalConvBlock(paddle.nn.Layer):
    """
    ...
    def __init__(self, in_channels, out_channels=None, dropout=0.0, spatial_aware=False):
        ...
        self.conv1 = paddle.nn.Sequential(
            paddle.nn.GroupNorm(num_groups=32, num_channels=in_channels),
            paddle.nn.Silu(),
            paddle.nn.Conv3D(
                in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_shape, padding=padding_shape
            ),
        )
        self.conv2 = paddle.nn.Sequential(
            paddle.nn.GroupNorm(num_groups=32, num_channels=out_channels),
            paddle.nn.Silu(),
            paddle.nn.Dropout(p=dropout),
            paddle.nn.Conv3D(
                in_channels=out_channels, out_channels=in_channels, kernel_size=kernel_shape, padding=padding_shape
            ),
        )
        self.conv3 = paddle.nn.Sequential(
            ...
            paddle.nn.Dropout(p=dropout),
            paddle.nn.Conv3D(
                in_channels=out_channels, out_channels=in_channels, kernel_size=(3, 1, 1), padding=(1, 0, 0)
            ),
        )
        ...
        init_Constant(self.conv4[-1].weight)
        init_Constant = paddle.nn.initializer.Constant(value=0.0)
        init_Constant(self.conv4[-1].bias)
...
class UNetModel(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.time_embed = paddle.nn.Sequential(
            linear(model_channels, time_embed_dim), paddle.nn.Silu(), linear(time_embed_dim, time_embed_dim)
        )
        if self.fps_cond:
            self.fps_embedding = paddle.nn.Sequential(
                linear(model_channels, time_embed_dim), paddle.nn.Silu(), linear(time_embed_dim, time_embed_dim)
            )
        self.input_blocks = paddle.nn.LayerList(
            sublayers=[TimestepEmbedSequential(conv_nd(dims, in_channels, model_channels, 3, padding=1))]
        )
        ...
        self.out = paddle.nn.Sequential(
            normalization(ch), paddle.nn.Silu(), zero_module(conv_nd(dims, model_channels, out_channels, 3, padding=1))
        )

    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        t_emb = t_emb.astype(self.time_embed._dtype)
        x = x.astype(self._dtype)
        emb = self.time_embed(t_emb)
        if self.fps_cond:
            if type(fps) == int:
                fps = paddle.full_like(x=timesteps, fill_value=fps)
            fps_emb = timestep_embedding(fps, self.model_channels, repeat_only=False)
            fps_emb = fps_emb.astype(self.fps_embedding._dtype)
