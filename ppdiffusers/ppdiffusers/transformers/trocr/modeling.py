...
from .configuration import TrOCRConfig
...
class TrOCRSinusoidalPositionalEmbedding(nn.Layer):
    ...
    @paddle.no_grad()
    def forward(self, input_ids: paddle.Tensor, past_key_values_length: int = 0):
        bsz, seq_len = tuple(input_ids.shape)
        ...
        x = self.weights.index_select(0, position_ids.reshape([-1])).reshape([bsz, seq_len, -1]).detach()
...
class TrOCRAttention(nn.Layer):
    ...
    def forward(
        ...
        bsz, tgt_len, embed_dim = tuple(hidden_states.shape)
        ...
        attn_weights = paddle.bmm(query_states, key_states.transpose(dim2perm(key_states.ndim, 1, 2)))

        if tuple(attn_weights.shape) != (bsz * self.num_heads, tgt_len, src_len):
            raise ValueError(
        ...
        if attention_mask is not None:
            if tuple(attention_mask.shape) != (bsz, 1, tgt_len, src_len):
                raise ValueError(
        ...
        if tuple(attn_output.shape) != (bsz * self.num_heads, tgt_len, self.head_dim):
            raise ValueError(
        ...
        attn_output = attn_output.transpose(dim2perm(attn_output.ndim, 1, 2))
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Conv1D)):
            ...
            if module._padding_idx is not None:
                ...
                init_constant(module.weight.data[module._padding_idx])
...
class TrOCRDecoder(TrOCRPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            input = input_ids
            input_ids = input_ids.reshape([-1, input.shape[-1]])
...
class TrOCRForCausalLM(TrOCRPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.output_projection = nn.Linear(config.hidden_size, config.vocab_size, bias_attr=False)
...
TrOCRForCausalLMModel = TrOCRForCausalLM  # for compatibility, paddle need to add "Model" suffix to the model name
__all__ = ["TrOCRForCausalLM", "TrOCRPreTrainedModel", "TrOCRForCausalLMModel"]
