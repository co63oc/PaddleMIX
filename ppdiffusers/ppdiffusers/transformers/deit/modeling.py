...
from .configuration import DeiTConfig
...
def prune_linear_layer(layer: nn.Linear, index: paddle.Tensor, dim: int = 0) -> nn.Linear:
    ...
    new_layer = nn.Linear(new_size[1], new_size[0], bias_attr=layer.bias is not None).to(layer.weight.device)
...
class DeiTPatchEmbeddings(nn.Layer):
    ...
    def forward(self, pixel_values: paddle.Tensor) -> paddle.Tensor:
        ...
        x = x.transpose(dim2perm(x.ndim, 1, 2))
        return x
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights = paddle.matmul(query, key.transpose(dim2perm(key.ndim, -2, -1))) * scaling
    ...
    attn_output = attn_output.transpose(dim2perm(attn_output.ndim, 2, 1))
...
class DeiTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[nn.Linear, nn.Conv2D, nn.LayerNorm]) -> None:
        ...
        init_constant_1 = paddle.nn.initializer.Constant(value=1.0)
        if isinstance(module, (nn.Linear, nn.Conv2D)):
            ...
            init_TruncatedNormal(
                module.weight.data
            )
            if module.bias is not None:
                module.bias.data.stop_gradient = True
                module.bias.data.zero_()
                module.bias.data.stop_gradient = False
        elif isinstance(module, nn.LayerNorm):
            init_constant(module.bias)
            init_constant_1(module.weight)
        elif isinstance(module, DeiTEmbeddings):
            init_constant(module.cls_token)
            init_constant(module.position_embeddings)
            init_constant(module.distillation_token)
            if module.mask_token is not None:
                init_constant(module.mask_token)
