...
class AttentionMaskConverter:
    ...
    >>> converter.to_4d(paddle.Tensor([[0, 0, 0, 1, 1]]), 5, key_value_length=5, dtype=paddle.float32)
    ...
    def to_4d(
        ...
        if causal_4d_mask is not None:
            expanded_attn_mask = causal_4d_mask.masked_fill(expanded_attn_mask.bool(), paddle.finfo(dtype).min)
    ...
    def _make_causal_mask(
        ...
        if past_key_values_length > 0:
            mask = paddle.cat([paddle.zeros(tgt_len, past_key_values_length, dtype=dtype), mask], axis=-1)

        # add lower triangular sliding window mask if necessary
        if sliding_window is not None:
            diagonal = past_key_values_length - sliding_window - 1

            context_mask = paddle.tril(paddle.ones_like(mask, dtype=paddle.bool), diagonal=diagonal)
    ...
    def _unmask_unattended(
        ...
        return expanded_mask.mul(~paddle.all(expanded_mask == min_dtype, axis=-1, keepdim=True))
    ...
    def _ignore_causal_mask_sdpa(
        ...
        ignored in case Paddle's SDPA is used, rather relying on SDPA's `is_causal` argument.
        ...
        if attention_mask is None:
            ...
            if len(attention_mask.shape) == 4:
                return False
            elif not is_tracing and paddle.all(attention_mask == 1):
...
def _prepare_4d_attention_mask(mask: paddle.Tensor, dtype: paddle.dtype, tgt_len: Optional[int] = None):
    """
    ...
    Args:
        mask (`paddle.Tensor`):
            A 2D attention mask of shape `(batch_size, key_value_length)`
        dtype (`paddle.dtype`):
            The paddle dtype the created mask shall have.
        tgt_len (`int`):
...
def _prepare_4d_attention_mask_for_sdpa(mask: paddle.Tensor, dtype: paddle.dtype, tgt_len: Optional[int] = None):
    """
    Creates a non-causal 4D mask of shape `(batch_size, 1, query_length, key_value_length)` from a 2D mask of shape
    `(batch_size, key_value_length)`
    ...
    if not is_tracing and paddle.all(mask == 1):
        return None
    else:
        return AttentionMaskConverter._expand_mask(mask=mask, dtype=dtype, tgt_len=tgt_len)
